//
//  DIDIRecognizerViewParamsObject.h
//  DIDIVoiceRecognitionClient
//
//  Created by DD on 13-9-25.
//  Copyright (c) 2013年 DD, Inc. All rights reserved.
//
...
typedef enum
{
    DDRecognizerResultShowModeNotShow     = 0,      // 不显示结果
    DDRecognizerResultShowModeWholeShow,            // 仅显示最终结果
    DDRecognizerResultShowModeContinuousShow,       // 提供连续上屏效果（默认）
} TDDRecognizerResultShowMode;

// 枚举 - 播放提示音开关
typedef enum
{
    EDDRecognizerPlayTonesRecordForbidden = 0,                  // 不开启提示音
    EDDRecognizerPlayTonesRecordPlay = 1,                          //  提示音开启
} TDDRecognizerPlayTones;

// 枚举 - 设置识别语言
typedef enum
{
    DDVoiceRecognitionLanguageChinese = 0,
    DDVoiceRecognitionLanguageCantonese,
    DDVoiceRecognitionLanguageEnglish,
} TDDVoiceRecognitionLanguage;

// @class - DIDIRecognizerViewParamsObject
// @brief - 语音识别弹窗参数配置类
@interface DIDIRecognizerViewParamsObject : NSObject

@property (nonatomic, copy) NSString *productID;  // 向DIDI语音技术部申请
@property (nonatomic, copy) NSString *apiKey;  // 开发者apiKey，在DIDI开放平台申请
@property (nonatomic, copy) NSString *secretKey;  // 开发者secretKey，在DIDI开放平台申请
@property (nonatomic) BOOL isNeedNLU;  // 是否请求语义解析，只对搜索模式有影响
@property (nonatomic) TDDVoiceRecognitionLanguage language;   // 识别语言
@property (nonatomic) TDDVoiceRecognitionProperty recognitionProperty DEPRECATED_ATTRIBUTE;  // 识别模式
@property (nonatomic, copy) NSArray *recogPropList; // 识别模式
@property (nonatomic) NSInteger cityID; // 城市标识
@property (nonatomic) TDDVoiceRecognitionResourceType resourceType; // 资源类型
...
@property (nonatomic) TDDRecognizerResultShowMode resultShowMode;   // 显示效果
@property (nonatomic) TDDRecognizerPlayTones recordPlayTones;   // 提示音开关
...
@end // DIDIRecognizerViewParamsObject
