...
typedef struct
{
   Float32 bckr_est[COMPLEN_NB];   /* background noise estimate */
   Float32 ave_level[COMPLEN_NB];


   /* averaged input components for stationary estimation */
   Float32 old_level[COMPLEN_NB];   /* input levels of the previous frame */
   Float32 sub_level[COMPLEN_NB];
...
typedef struct
{
   Float32 lsp_hist[M_NB * DTX_HIST_SIZE];
...
typedef struct
{
   Float32 past_rq[M_NB];   /* Past quantized prediction error */
...
{
   /* Past LSPs */
   Float32 lsp_old[M_NB];
   Float32 lsp_old_q[M_NB];
...
typedef struct
{
   Float32 old_A[M_NB + 1];   /* Last A(z) for case of unstable filter */
...
typedef struct
{
   ...
   /* Excitation vector */
   Float32 old_exc[L_FRAME_NB + PIT_MAX_NB + L_INTERPOL_NB];
   Float32 *exc;


   /* Zero vector */
   Float32 ai_zero[L_SUBFR_NB + MP1];
   Float32 *zero;


   /* Impulse response vector */
   Float32 *h1;
   Float32 hvec[L_SUBFR_NB * 2];
   ...
   /* Filter's memory */
   Float32 mem_syn[M_NB], mem_w0[M_NB], mem_w[M_NB];
   Float32 mem_err[M_NB + L_SUBFR_NB], *error;
   Float32 sharp;
...
{
   Word32 i, j;   /* Counters */
   Float32 y[L_WINDOW_NB + M_NB + 1];   /* Windowed signal */
   Float64 sum;   /* temp */
...
static void Levinson( Float32 *old_A, Float32 *r, Float32 *A, Float32 *rc )
{
   Float32 sum, at, err;
   Word32 l, j, i;
   Float32 rct[M_NB];   /* temporary reflection coefficients  0,...,m-1 */
...
static void Az_lsp( Float32 a[], Float32 lsp[], Float32 old_lsp[] )
{
   ...
   for ( i = 0; i < ( NC ); i++ ) {
      f1[i + 1] = a[i + 1] +a[M_NB - i] - f1[i];
      f2[i + 1] = a[i + 1] -a[M_NB - i] + f2[i];
   }
   f1[NC] *= 0.5F;
   f2[NC] *= 0.5F;
...
{
   Float32 lsf1[M_NB], wf1[M_NB], lsf_p[M_NB], lsf_r1[M_NB];
   Float32 lsf1_q[M_NB];
   Float32 pred_init_err;
   Float32 min_pred_init_err;
   Float32 temp_r1[M_NB];
   Float32 temp_p[M_NB];
   Word32 j, i;
   ...
   if ( mode != MRDTX ) {
      ...
      *pred_init_i = 0;
      ...
      for ( j = 0; j < PAST_RQ_INIT_SIZE; j++ ) {
         ...
         for ( i = 0; i < M_NB; i++ ) {
            temp_p[i] = mean_lsf_3[i] + past_rq_init[j * M_NB + i];
         ...
         if ( pred_init_err < min_pred_init_err ) {
            ...
            memcpy( lsf_r1, temp_r1, M_NB <<2 );
            memcpy( lsf_p, temp_p, M_NB <<2 );
            memcpy( past_rq, &past_rq_init[j * M_NB], M_NB <<2 );
            *pred_init_i = j;
         }
      }
   }
...
{
   Float32 lsf1[M_NB], lsf2[M_NB], wf1[M_NB], wf2[M_NB], lsf_p[M_NB], lsf_r1[M_NB], lsf_r2[M_NB];
   Float32 lsf1_q[M_NB], lsf2_q[M_NB];
   Word32 i;
...
static void lsp( enum Mode req_mode, enum Mode used_mode, Float32 *lsp_old,
      ...
      lsp_new[], Word16 **anap )
{
   Float32 lsp_new_q[M_NB];   /* LSPs at 4th subframe */
   Float32 lsp_mid[M_NB], lsp_mid_q[M_NB];   /* LSPs at 2nd subframe */
   ...
   memcpy( lsp_old, lsp_new, M_NB <<2 );
   memcpy( lsp_old_q, lsp_new_q, M_NB <<2 );
}
...
#endif
{
   ...
   for ( i = lag_max, j = ( PIT_MAX_NB - lag_max - 1 ); i >= lag_min; i--, j-- ) {
      if ( corr[ - i] >= max ) {
         max = corr[ - i];
         p_max = i;
      }
      ...
      Word16 idx, Word32 dtx )
{
   ...
   corrPtr = &corr[PIT_MAX_NB];
   comp_corr( signal, L_FRAME_BY2, PIT_MAX_NB, PIT_MIN, corrPtr );
   ...
   if ( dtx ) {
      if ( idx == 1 ) {
         /* calculate max high-passed filtered correlation of all lags */
         hp_max( corrPtr, signal, L_FRAME_BY2, PIT_MAX_NB, PIT_MIN, &corr_hp_max );
      ...
      , Word32 *old_lags, Word32 dtx, Word16 idx )
{
   ...
   if ( ( mode == MR475 ) || ( mode == MR515 ) ) {
      *T_op = Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX_NB, L_FRAME_NB, dtx, idx );
   }
   else {
      if ( mode <= MR795 ) {
         *T_op = Pitch_ol( mode, vadSt, wsp, PIT_MIN, PIT_MAX_NB, L_FRAME_BY2, dtx,
               idx );
      }
      else if ( mode == MR102 ) {
         ...
         *T_op = Pitch_ol( mode, vadSt, wsp, PIT_MIN_MR122, PIT_MAX_NB, L_FRAME_BY2
               , dtx, idx );
      }
   }
}
      ...
      Float32 *gp_limit )
{
   ...
   exc_tmp_p = exc_tmp + PIT_MAX_NB + L_INTERPOL_NB;
   ...
   *( *anap )++ = ( Word16 )i;
    ...
    * syncronisation
    */
   for (i = -(PIT_MAX_NB + L_INTERPOL_NB); i < 40; i++)
      ...
      Float32 *mem_w0, Float32 *exc, Float32 *sharp )
{
   ...
   *sharp = gain_pit;
   ...
   for ( i = L_SUBFR_NB - M_NB, j = 0; i < L_SUBFR_NB; i++, j++ ) {
      /* e(n) = S(n) - S_s(n) */
      ...
      Float32 speech[], Float32 *log_en_hist )
{
   ...
   *hist_ptr += 1;
   ...
   memcpy( &lsp_hist[ * hist_ptr * M_NB], lsp_new, sizeof( Float32 )*M_NB );
...
{
   Float32 log_en, lsf[M_NB], lsp[M_NB], lsp_q[M_NB];
   Word32 i, j;
    ...
    */
      log_en = 0;
      memset( lsp, 0, sizeof( Float32 )*M_NB );
 ...
 */
#ifndef VAD2
static Word16 vad_decision( vadState *st, Float32 level[COMPLEN_NB], Float32
      pow_sum )
{
      ...
      Word16 ana[], enum Mode *used_mode, Float32 synth[] )
{
   ...
   Float32 lsp_new[M_NB];


   /* Other vectors */
   Float32 xn[L_SUBFR_NB];   /* Target vector for pitch search */
   Float32 xn2[L_SUBFR_NB];   /* Target vector for codebook search */
   Float32 code[L_SUBFR_NB];   /* Fixed codebook excitation */
   Float32 y1[L_SUBFR_NB];   /* Filtered adaptive excitation */
   Float32 y2[L_SUBFR_NB];   /* Filtered fixed codebook excitation */
   Float32 gCoeff[3];   /* Correlations between xn, y1, & y2: */
   Float32 res[L_SUBFR_NB];   /* Short term (LPC) prediction residual */
   Float32 res2[L_SUBFR_NB];   /* Long term (LTP) prediction residual */


   /* Vector and scalars needed for the MR475 */
   Float32 xn_sf0[L_SUBFR_NB];   /* Target vector for pitch search */
   Float32 y2_sf0[L_SUBFR_NB];   /* Filtered codebook innovation */
   Float32 code_sf0[L_SUBFR_NB];   /* Fixed codebook excitation */
   Float32 h1_sf0[L_SUBFR_NB];   /* The impulse response of sf0 */
   Float32 mem_syn_save[M_NB];   /* Filter memory */
   Float32 mem_w0_save[M_NB];   /* Filter memory */
   Float32 mem_err_save[M_NB];   /* Filter memory */
   ...
   memcpy( st->new_speech, new_speech, L_FRAME_NB <<2 );
   ...
   if ( *used_mode == MRDTX ) {
      ...
      memset( st->old_exc, 0, ( PIT_MAX_NB + L_INTERPOL_NB )<<2 );
      memset( st->mem_w0, 0, M_NB <<2 );
      memset( st->mem_err, 0, M_NB <<2 );
      memset( st->zero, 0, L_SUBFR_NB <<2 );
      memset( st->hvec, 0, L_SUBFR_NB <<2 );
      memset( st->lspSt->qSt->past_rq, 0, M_NB <<2 );
      memcpy( st->lspSt->lsp_old, lsp_new, M_NB <<2 );
      memcpy( st->lspSt->lsp_old_q, lsp_new, M_NB <<2 );
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME_NB; i_subfr += L_SUBFR_NB ) {
      subfrNr += 1;
      evenSubfr = 1 - evenSubfr;

      if ( ( evenSubfr != 0 ) && ( *used_mode == MR475 ) ) {
         memcpy( mem_syn_save, st->mem_syn, M_NB <<2 );
         memcpy( mem_w0_save, st->mem_w0, M_NB <<2 );
         memcpy( mem_err_save, st->mem_err, M_NB <<2 );
         sharp_save = st->sharp;
      }
      ...
      /* MR475 */
         ...
         if ( evenSubfr != 0 ) {
            memcpy( h1_sf0, st->h1, L_SUBFR_NB <<2 );
         }
      }

      /* copy the LP residual (res2 is modified in the CL LTP search) */
      memcpy( res2, res, L_SUBFR_NB <<2 );
      ...
      if ( *used_mode != MR475 ) {
         ...
         if ( evenSubfr != 0 ) {
            i_subfr_sf0 = i_subfr;
            memcpy( xn_sf0, xn, L_SUBFR_NB <<2 );
            memcpy( y2_sf0, y2, L_SUBFR_NB <<2 );
            memcpy( code_sf0, code, L_SUBFR_NB <<2 );
            T0_sf0 = T0;
            T0_frac_sf0 = T0_frac;
             ...
             */
            memcpy( st->mem_err, mem_err_save, M_NB <<2 );
...
the_end:
   ...
   for ( i = 0; i < PIT_MAX_NB; i++ ) {
      st->old_wsp[i] = st->old_wsp[L_FRAME_NB + i];
   }

   for ( i = 0; i < PIT_MAX_NB + L_INTERPOL_NB; i++ ) {
      st->old_exc[i] = st->old_exc[L_FRAME_NB + i];
   }

   for ( i = 0; i < L_TOTAL_NB - L_FRAME_NB; i++ ) {
      st->old_speech[i] = st->old_speech[L_FRAME_NB + i];
   }
}
...
static void cod_amr_reset( cod_amrState *s, Word32 dtx )
{
   ...
   /* reset Q_plsfState */
   memset( s->lspSt->qSt->past_rq, 0, sizeof( Float32 )*M_NB );
   ...
   memset( &s->lpcSt->LevinsonSt->old_A[1], 0, sizeof( Float32 )*M_NB );
   ...
   for ( i = 0; i < DTX_HIST_SIZE; i++ ) {
      memcpy( &s->dtxEncSt->lsp_hist[i * M_NB], lsp_init_data, sizeof( Float32 )*M_NB
            );
   }
   memset( s->dtxEncSt->log_en_hist, 0, M_NB * sizeof( Float32 ) );
   s->dtxEncSt->dtxHangoverCount = DTX_HANG_CONST;
   s->dtxEncSt->decAnaElapsedCount = DTX_ELAPSED_FRAMES_THRESH;

   /* init speech pointers */
   /* New speech */
   s->new_speech = s->old_speech + L_TOTAL_NB - L_FRAME_NB;

   /* Present frame */
   s->speech = s->new_speech - L_NEXT;
   s->p_window = s->old_speech + L_TOTAL_NB - L_WINDOW_NB;

   /* For LPC window				*/
   s->p_window_12k2 = s->p_window - L_NEXT;

   /* Initialize static pointers */
   s->wsp = s->old_wsp + PIT_MAX_NB;
   s->exc = s->old_exc + PIT_MAX_NB + L_INTERPOL_NB;
   s->zero = s->ai_zero + MP1;
   s->error = s->mem_err + M_NB;
   s->h1 = &s->hvec[L_SUBFR_NB];

   /* Static vectors to zero */
   memset( s->old_speech, 0, sizeof( Float32 )*L_TOTAL_NB );
   memset( s->old_exc, 0, sizeof( Float32 )*( PIT_MAX_NB + L_INTERPOL_NB ) );
   memset( s->old_wsp, 0, sizeof( Float32 )*PIT_MAX_NB );
   memset( s->mem_syn, 0, sizeof( Float32 )*M_NB );
   memset( s->mem_w, 0, sizeof( Float32 )*M_NB );
   memset( s->mem_w0, 0, sizeof( Float32 )*M_NB );
   memset( s->mem_err, 0, sizeof( Float32 )*M_NB );
   memset( s->ai_zero, 0, sizeof( Float32 )*L_SUBFR_NB );
   memset( s->hvec, 0, sizeof( Float32 )*L_SUBFR_NB );
...
void * Speech_Encode_Frame_init( int dtx )
{
   ...
   if ( Pre_Process_init( &s->pre_state ) || cod_amr_init( &s->cod_amr_state, dtx ) ) 
   {
...
{
   Float32 syn[L_FRAME_NB];   /* Buffer for synthesis speech */
   Float32 speech[160];
   Word32 i;
